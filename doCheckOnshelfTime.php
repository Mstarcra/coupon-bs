...
$sql="SELECT id, created_time, onshelf_time FROM instrument";
...
foreach($rows as $row){
    ...
    if($created_time>$onshelf_time){
        ...
        $newTime=date("Y-m-d H:i:s", strtotime("+2 day", $onshelf_time));

        // 更改資料前先echo一次，確定時間如自己想的變動，可用id確認是哪一筆資料
        // echo $row["id"].":".$newTime."<br>";
        $sql="UPDATE instrument SET onshelf_time = '$newTime' WHERE id = $id";
